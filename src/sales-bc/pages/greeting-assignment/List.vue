...
onMounted(async () => {
  await viewSummary()
  tab.value = route.query.tab as string ?? sessionStorage.getItem('greeting-assignment-tab-state') ?? "new-assignment"
})

watch(tab, (newTab) => {
  sessionStorage.setItem('greeting-assignment-tab-state', newTab);
})
...
const viewSummary = async () => {
  ...
  const response = await salesRepository.getUser()
    .executeSalesGraphqlQuery<ResponseType>([
      {
        operation: { name: "totalGreetingAssignment", alias: "customerAssignmentCount" },
        variables: {},
        fields: []
      },
      {
        operation: { name: "totalGreetingAssignment", alias: "newAssignmentCount" },
        variables: {
      ...
      {
        operation: { name: "totalGreetingAssignment", alias: "idleAssignmentCount" },
        variables: {
      ...
      {
        operation: { name: "totalGreetingAssignment", alias: "activeAssignmentCount" },
        variables: {
          activeFilters: {
            type: "[FilterInput]", name: "filters",
