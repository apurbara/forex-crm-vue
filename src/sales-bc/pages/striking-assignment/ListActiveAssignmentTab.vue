...
const StrikingAssignmentPagination = reactive(new OffsetPagination<StrikingAssignmentType>(
  ...
  undefined,
  'striking-assignment-active-page-state'
))

onMounted(async () => {
  StrikingAssignmentPagination.addHiddenFilter({ column: 'StrikingAssignment.status', value: CustomerAssignmentStatus.ACTIVE });
  await StrikingAssignmentPagination.loadPage();
})
