<template>
  <section class="page-section ma-2">
    ...
    <div v-if="!editingCustomerBio" class="d-flex align-center justify-start flex-wrap">
      ...
      <InfoComponentIcon v-if="!editingCustomerBio" style="min-width: 40%;"
        :info="{ value: customerAssignment.customer.city?.name, icon: `mdi-map-marker-outline` }" />
    </div>
    <div v-else>
      ...
      <v-autocomplete label="city" variant="outlined" :items="cityList" density="compact" item-title="name"
        return-object v-model="customerAssignment.customer.city" />
    </div>
