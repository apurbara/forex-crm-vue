<template>
  <v-btn v-if="customerAssignment.canSubmitInitialReport()" @click="displaySalesActivityReportForm = true">
  ...
  <v-dialog v-model="displaySalesActivityReportForm" width="600px">
    <v-card>
      <v-card-title class="mt-6 text-center">Submit Initial Sales Activity Report</v-card-title>
      <v-card-text class="mt-4">
        ...
        <SalesActivityReportComponent :sales-activity-report="initialSalesActivityReport" />
      </v-card-text>
      <div class="d-flex justify-center ma-6">
        <v-btn block @click="submitSalesActivityReport" variant="tonal"
          :disabled="!initialSalesActivityReport.isValidToSubmit()">submit</v-btn>
      </div>
    </v-card>
  </v-dialog>
...
const initialSalesActivityReport = reactive<SalesActivityReport>(new SalesActivityReport())
...
const submitSalesActivityReport = async () => {
  type ResponseType = { updateCustomerAssignmentJourney: CustomerAssignmentType, submitInitialSalesActivityReport: SalesActivityScheduleType }
    ...
    .executeSalesGraphqlMutation<ResponseType>(httpRequest, [
      ...
      },
      {
        ...
        variables: {
          ...
          content: initialSalesActivityReport.content
        },
        fields: [
  ...
  const initialSalesActivitySchedule = new SalesActivitySchedule();
  initialSalesActivitySchedule.load(response.submitInitialSalesActivityReport)
  props.customerAssignment.salesActivitySchedules.push(initialSalesActivitySchedule)
