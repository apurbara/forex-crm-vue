    ...
    >
      ...
      <tbody>
        <tr
          ...
          ) in customerAssignment.customer.getUnverifiedReportList()"
          :key="verificationReport.id ?? index"
        >
          <td>
            <p class="font-14">
              {{ limitString(verificationReport.customerVerification?.name, 30) }}
            </p>
            <p class="font-12">
              {{ limitString(verificationReport.customerVerification?.description, 40) }}
            </p>
          </td>
          <td>
          ...
          <td>
            <v-btn
              ...
              size="small"
              @click="showVerificationReportSubmission(verificationReport)"
            ></v-btn>
            <v-btn
              variant="text"
              icon="mdi-check-circle-outline"
              size="small"
              @click="submitVerificationReport(verificationReport)"
            ></v-btn>
          </td>
        </tr>
      </tbody>
...
onMounted(async () => {
  const response = await companyUserRepository.getUser()!.executeGraphqlQueryInCompany<{
    ...
    variables: {
      filters: {
        type: "[FilterInput]",
        value: [{ column: "CustomerVerification.disabled", value: false }],
      },
    },
