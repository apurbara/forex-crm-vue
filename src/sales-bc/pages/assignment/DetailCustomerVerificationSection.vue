<template>
  <section class="page-section ma-2">
    <div class="d-flex justify-space-between">
      ...
      <v-chip>{{ customerAssignment.customer?.countTotalVerificationWeight() }}</v-chip>
    </div>
...
import VerificationReport, { VerificationReportType } from '@/sales-bc/domain/model/sales/customer-assignment/customer/verification-report';
...
onMounted(async () => {
  const response = await companyUserRepository.getUser()
    .executeGraphqlQueryInCompany<{ viewAllActiveCustomerVerification: CustomerVerificationType[] }>(httpRequest, {
