...
const selectedSalesActivity = ref<SalesActivityType>();
...
const submitReportAndUpdateJourney = async () => {
  ...
  const operations: GraphqlBuilderOptions[] = [
    {
      ...
      variables: {
        ...
        SalesActivity_id: { type: "ID", value: selectedSalesActivity.value?.id },
        ...salesActivityReport.toGraphqlVariables(),
      },
      fields: [
        ...["id", "submitTime", "content"],
        {
          salesActivitySchedule: ["id", "status", { salesActivity: ["id", "name", "description"] }],
        },
      ],
    },
  ];
  ...
  const response = await salesRepository.getUser().executeSalesGraphqlMutation<{
    submitNonScheduledSalesActivityReport: SalesActivityReportType;
    updateCustomerAssignmentJourney?: CustomerAssignmentType;
  }>(operations);

  salesActivityReport.load(response.submitNonScheduledSalesActivityReport);
  const salesActivitySchedule = new SalesActivitySchedule(
    response.submitNonScheduledSalesActivityReport.salesActivitySchedule
  );
