  ...
  <div class="layout">
    <v-app-bar app absolute :clipped-left="true" elevation="1" color="transparent" v-bind="$attrs">
      ...
      <AppBarMenuItemComponent
        v-for="(appBarMenuItem, key) in layout.appBarMenuItems"
        :key="key"
...
const props = withDefaults(defineProps<{ layout: LayoutInterface; showLogout?: boolean }>(), {
  showLogout: true,
});
// const { smAndDown, lgAndUp } = useDisplay()
// const navBarShowStatus = ref(!smAndDown.value);
const { companyUserRepository, adminRepository, managerRepository, salesRepository } =
