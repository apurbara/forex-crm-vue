...
import ManagerRole from '../role/manager-role';
...
const login = async () => {
	const response = await userRepository
		.getUser()
		.executeGraphqlMutation<{ managerLogin: UserRoleType }>(httpRequest, {
			operation: 'managerLogin',
			variables: loginPayload.toGraphqlVariable(),
			fields: ['token', 'name'],
		});
	const managerData = { ...response.managerLogin, type: ManagerRole.type };
	userRepository.logUserIn(managerData)
