<template>
  <div>
    <h1 class="page-title">Distribute Greeting Assignment</h1>
...
const salesPagination = reactive(new OffsetPagination<SalesType>(
  ...
  .addHiddenFilter({ column: "Sales.role", value: SalesRoleEnum.GREETER, comparisonType: 'EQ' })
)
...
const submit = async () => {
  await managerRepository.getUser()!
    .executeManagerGraphqlMutation({
      operation: 'assignGreetingActivityOfCustomerListToSales',
      variables: {
  ...
  router.push(`/manager-greeting-assignment`)
}
