...
const customerAssignmentPagination = reactive(new OffsetPagination<GreetingAssignmentType>(
  async (pagination) => {
    const response = await managerRepository.getUser()
      .executeManagerGraphqlQuery<{ greetingAssignmentList: PaginationResponseType<GreetingAssignmentType> }>({
...
const toDetail = (greetingAssignmentId: string) => router.push(`/manager-greeting-assignment/${greetingAssignmentId}`)
