<template>
  ...
  <OffsetPaginationComponent :pagination="pagination">
    <template v-slot:editSection>
      <v-btn prepend-icon="mdi-account-plus-outline" variant="tonal" to="/recycle-request/add">Add
        Recycle Request</v-btn>
    </template>
...
const pagination = reactive(new OffsetPagination<RecycleRequestType>(
  ...
  [
    new EnumFilter('status', 'RecycleRequest.status', () => Object.keys(ManagementApprovalStatus).map(key => { return { name: key, value: key } }), 'IN', 'name', 'value'),
  ],
  new KeywordSearch(["RecycleRequest.name", "RecycleRequest.description"])
))
...
const approveConfirmation = (event: Event, recycleRequest: RecycleRequestType) => {
  confirm.require({
    ...
    accept: async () => {
      const response = await companyUserRepository.getUser()
        .executeGraphqlMutationInCompany<{ acceptRecycleRequest: RecycleRequestType }>(httpRequest, {
          operation: "acceptRecycleRequest",
...
const rejectConfirmation = (event: Event, recycleRequest: RecycleRequestType) => {
  confirm.require({
    ...
    accept: async () => {
      const response = await companyUserRepository.getUser()
        .executeGraphqlMutationInCompany<{ rejectRecycleRequest: RecycleRequestType }>(httpRequest, {
