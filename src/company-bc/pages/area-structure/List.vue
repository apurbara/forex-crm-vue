    ...
    <v-table height="400px" density="compact" style="width: 100%;" class="datatable">
      ...
      <tbody>
        ...
        <tr v-else v-for="(areaStructure, index) in pagination.resultList" :key="areaStructure.id ?? index"
          ...
          <td>{{ new Date(areaStructure.createdTime!).toLocaleDateString() }}</td>
          <td>{{ areaStructure.parent?.name }}</td>
          <td>
            <v-btn v-if="!areaStructure.disabled" variant="text" icon="mdi-close-circle-outline" size="x-small"
              @click="disableConfirmation($event, areaStructure.id!)"></v-btn>
...
import { AreaStructureType } from '@/company-bc/domain/model/area-structure';
...
const pagination = reactive(new OffsetPagination<AreaStructureType>(
  async (pagination) => {
    const response = await companyUserRepository.getUser()
      .executeGraphqlQueryInCompany<{ areaStructureList: PaginationResponseType<AreaStructureType> }>(httpRequest, {
...
const disableConfirmation = (event: Event, areaStructureId: string) => {
  confirm.require({
    ...
    accept: async () => {
      const response = await companyUserRepository.getUser()
        .executeGraphqlMutationInCompany<{ disableAreaStructure: AreaStructureType }>(httpRequest, {
          operation: "disableAreaStructure",
          variables: { id: { type: "ID", value: areaStructureId } },
          fields: ['disabled']
        })
      pagination.resultList.find((areaStructure: AreaStructureType) => areaStructure.id === areaStructureId)!.disabled = response.disableAreaStructure.disabled
    },
    reject: () => { }
  });
};
