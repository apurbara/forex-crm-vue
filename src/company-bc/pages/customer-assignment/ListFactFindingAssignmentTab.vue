  ...
  <OffsetPaginationComponent :pagination="pagination">
    <template v-slot:editSection>
      <v-btn prepend-icon="mdi-store-plus-outline" class="ml-4" variant="tonal"
        to="/customer-assignment/distribute-fact-finding-assignment">
...
const customerStatusFilter = new EnumFilter('status', 'FactFindingAssignment.status', () => Object.keys(CustomerAssignmentStatus).map(key => { return { label: key, value: key } }), 'select status...');
customerStatusFilter.selectedItems.push({ value: CustomerAssignmentStatus.ACTIVE, label: CustomerAssignmentStatus.ACTIVE })
const pagination = reactive(new OffsetPagination<FactFindingAssignmentType>(
  async (pagination) => {
    const response = await companyUserRepository.getUser()!
      .executeGraphqlQueryInCompany<{ factFindingAssignmentList: PaginationResponseType<FactFindingAssignmentType> }>({
