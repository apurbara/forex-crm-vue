<template>
  <div>
    <h1 class="page-title">Distribute Fact Finding Assignment</h1>
...
const salesPagination = reactive(new OffsetPagination<SalesType>(
  ...
  .addHiddenFilter({ column: "Sales.role", value: SalesRoleEnum.FACT_FINDER, comparisonType: 'EQ' })
)
...
const submit = async () => {
  await companyUserRepository.getUser()!
    .executeGraphqlMutationInCompany({
      operation: 'assignFactFindingActivityOfCustomerListToSales',
      variables: {
