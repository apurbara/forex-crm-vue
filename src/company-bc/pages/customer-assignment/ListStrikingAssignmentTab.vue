  ...
  <OffsetPaginationComponent :pagination="pagination">
    <template v-slot:editSection>
      <v-btn prepend-icon="mdi-store-plus-outline" class="ml-4" variant="tonal"
        to="/customer-assignment/distribute-striking-assignment">
        Distribute Striking Assignment</v-btn>
    </template>
...
const customerStatusFilter = new EnumFilter('status', 'StrikingAssignment.status', () => Object.keys(CustomerAssignmentStatus).map(key => { return { label: key, value: key } }), 'select status...');
customerStatusFilter.selectedItems.push({ value: CustomerAssignmentStatus.ACTIVE, label: CustomerAssignmentStatus.ACTIVE })
const pagination = reactive(new OffsetPagination<StrikingAssignmentType>(
  async (pagination) => {
    const response = await companyUserRepository.getUser()!
      .executeGraphqlQueryInCompany<{ strikingAssignmentList: PaginationResponseType<StrikingAssignmentType> }>({
