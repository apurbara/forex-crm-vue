<template>
  <div class="page-section">
    <h1 class="page-title">Add Individual Target</h1>
    <div class="form">
...
import CommonSalesMetric, {
  CommonSalesMetricType,
} from "@/company-bc/domain/model/common-sales-metric";
...
const submit = async () => {
  ...
  cache?.set(
    `common-sales-metric-${response?.createCommonSalesMetric.id}`,
    response?.createCommonSalesMetric
  );
  router.push(`/company/common-sales-metric/${response?.createCommonSalesMetric.id}`);
};
</script>
