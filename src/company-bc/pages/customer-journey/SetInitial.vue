...
const submit = async () => {
  ...
  cache?.set(
    `customer-journey-${response?.setInitialCustomerJourney.id}`,
    response?.setInitialCustomerJourney
  );
  router.push(`/company/customer-journey/${response?.setInitialCustomerJourney.id}`);
};
</script>
