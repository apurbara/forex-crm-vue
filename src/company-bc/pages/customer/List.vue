  ...
  <div class="page-section">
    ...
    <OffsetPaginationComponent :pagination="pagination">
        ...
        <tbody>
          <tr v-if="pagination.resultList.length < 1">
            <td class="no-data" colspan="12">
              <EmptyDataIllustrationComponent message="Data Customer Kosong" />
            </td>
          </tr>
      ...
      <template v-slot:navigation>
        <v-btn prepend-icon="mdi-account-plus-outline" variant="tonal" @click="exportCustomer"
          >export to scv</v-btn
        >
      </template>
...
const onFileChange = async (e: any) => {
  ...
  const failedList = await companyUserRepository
    .getUser<AdminRole>()!
    .uploadFile("/import-customer-from-csv", file, (event: AxiosProgressEvent) => {
