<template>
  <div>
    ...
    <div class="form">
      ...
      <CustomerVerificationComponent :customer-verification="customerVerification" :readonly="!editing" />
...
onMounted(async () => {
  cacheData = cache?.pull<CustomerVerificationType>(`customer-verification-${props.customerVerificationId}`);
  if (cacheData) {
    ...
    const response = await companyUserRepository.getUser()!
      .executeGraphqlQueryInCompany<{ customerVerificationDetail: CustomerVerificationType }>({
        ...
        fields: [
          'id', 'disabled', 'createdTime', 'name', 'description', 'position', 'weight'
        ],
      })
