...
const submit = async () => {
  ...
  cache?.set(`sales-activity-${response?.setInitialSalesActivity.id}`, response?.setInitialSalesActivity)
  router.push(`/sales-activity/${response?.setInitialSalesActivity.id}`)
}
