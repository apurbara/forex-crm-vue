  ...
  <OffsetPaginationComponent :pagination="pagination">
    ...
    <v-table height="400px" density="compact" style="width: 100%;" class="datatable">
      ...
      <tbody>
        ...
        <tr v-else v-for="(area, index) in pagination.resultList" :key="area.id ?? index"
          @dblclick="toDetail(area.id!)">
          <td>{{ area.name }}</td>
...
const pagination = reactive(new OffsetPagination<AreaType>(
  async (pagination) => {
    const response = await companyUserRepository.getUser()
      .executeGraphqlQueryInCompany<{ areaList: PaginationResponseType<AreaType> }>(httpRequest, {
        operation: 'areaList',
