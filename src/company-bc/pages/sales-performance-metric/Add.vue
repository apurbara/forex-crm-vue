<template>
  <div>
    <h1 class="page-title">Add Sales Performance Metric</h1>
    <div class="form">
      <SalesPerformanceMetricComponent :sales-performance-metric="salesPerformanceMetric" />
      <div class="d-flex justify-end">
        <v-btn :disabled="!salesPerformanceMetric.isValidProperties()" @click="submit">Submit</v-btn>
      </div>
    </div>
  </div>
</template>
...
const submit = async () => {
  const response = await companyUserRepository.getUser()
    .executeGraphqlMutationInCompany<{ createSalesPerformanceMetric: SalesPerformanceMetricType }>(httpRequest, {
  ...
  cache?.set(`sales-performance-metric-${response?.createSalesPerformanceMetric.id}`, response?.createSalesPerformanceMetric)
  router.push(`/sales-performance-metric/${response?.createSalesPerformanceMetric.id}`)
}
